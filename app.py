...
with st.sidebar:
    ...
    data_sheet_id  = st.text_input("Sheet ID data của bạn", placeholder="Paste Sheet ID")
    ...
    if st.button("🔌 Kết nối", type="primary", use_container_width=True):
        if uploaded_key and data_sheet_id:
            try:
                ...
                sm.init_sheets()
                # Load accounts ONCE right after connecting
...
if not st.session_state.connected:
    ...
    with st.expander("📖 Hướng dẫn setup"):
        st.markdown("""
1. Tạo **Google Cloud Project** → bật *Google Sheets API* + *Google Drive API*  
2. **Credentials → Service Account** → tab Keys → Add Key → JSON  
3. Tạo **Google Sheet mới** → copy Sheet ID từ URL  
4. **Share** cả 2 sheet cho email trong JSON (role Editor / Viewer)  
5. Upload JSON + nhập Sheet IDs bên trái → Kết nối  
        """)
    st.stop()
...
accounts           = st.session_state.accounts   # ← already loaded, no API call
...
with tab1:
    ...
    if not portfolio_df.empty:
        ...
        if "avg_buy_price" in disp.columns:
            disp["avg_buy_price"] = disp["avg_buy_price"].apply(lambda x: f"{x:,.0f}")
        if "current_price" in disp.columns:
...
with tab2:
    ...
    if not accounts:
        ...
        tax_rate      = 0.001  # 0.1% thuế TNCN khi bán
        ...
        with col2:
            ...
            if needs_price:
                ...
                if txn_type == "Mua":
                    default_fee = int(gross * buy_fee_rate)
                    txn_fee = st.number_input(
                        f"Phí mua (tự động {acc_info.get('buy_fee_rate','0.15')}%)",
                        value=default_fee, step=1000, key="txn_fee")
                    txn_tax = 0
                else:
                    default_fee = int(gross * sell_fee_rate)
                    default_tax = int(gross * tax_rate)
                    txn_fee = st.number_input(
                        f"Phí bán (tự động {acc_info.get('sell_fee_rate','0.15')}%)",
                        value=default_fee, step=1000, key="txn_fee")
                    txn_tax = st.number_input(
                        "Thuế TNCN (0.1%)", value=default_tax, step=1000, key="txn_tax")
        ...
        if st.button("💾 Lưu giao dịch", type="primary"):
            # Validation
            errors = []
            if needs_symbol and not txn_symbol:
                errors.append("Chưa nhập mã cổ phiếu")
            if needs_qty and txn_qty <= 0:
                errors.append("Số lượng phải > 0")
            if needs_price and txn_price <= 0:
                errors.append("Giá phải > 0")
            if needs_amount and txn_amount <= 0:
            ...
            if errors:
                ...
                try:
                    ...
                    final_tax    = txn_tax if needs_price else (txn_tax if txn_type == "Cổ tức tiền mặt" else 0)
                    ...
                    msg = logic.add_transaction(txn_data)
                    st.success(msg)
...
with tab3:
    ...
    if accounts:
        ...
        rename_map = {"name":"Tên TK","broker":"CTCK","buy_fee_rate":"Phí mua (%)","sell_fee_rate":"Phí bán (%)","note":"Ghi chú"}
        disp_cols  = [c for c in rename_map if c in acc_df.columns]
        st.dataframe(acc_df[disp_cols].rename(columns=rename_map), use_container_width=True, hide_index=True)
    else:
    ...
    with c2:
        new_buy_fee  = st.number_input("Phí mua (%)",  min_value=0.0, max_value=1.0, value=0.15, step=0.01, format="%.3f", key="new_buy_fee")
        new_sell_fee = st.number_input("Phí bán (%)",  min_value=0.0, max_value=1.0, value=0.15, step=0.01, format="%.3f", key="new_sell_fee")
    with c3:
        ...
        if st.button("➕ Thêm tài khoản", type="primary"):
            if new_name:
                sm.add_account({"name":new_name,"broker":new_broker,
                                ...
                                "note":new_note})
                # Reload accounts into session
                st.session_state.accounts = sm.get_accounts()
                st.success(f"✅ Đã thêm tài khoản '{new_name}'")
                st.rerun()
            else:
